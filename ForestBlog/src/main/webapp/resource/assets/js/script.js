$(document)
		.ready(
				...
				function() {
					...
					// 去边线
					$(
							".message-widget li:last, .message-page li:last, .hot_commend li:last, .search-page li:last, .my-comment li:last, .message-tab li:last")
							.css("border", "none");
...
(function($) {
	...
	function write() {
		var content = "<div class=\"ds-dialog\" id='PAY_"
				...
				+ "      </div>"
				+ "      <p class=\"qrcode-tip\">你的支持是我的动力</p>"
				+ "     </div>"
				...
				+ "      </div>" + "      <p class=\"qrcode-tip\">你的支持是我的动力</p>"
				+ "     </div>" + "     </div>" + "    </div>" + "   </div>"
				+ "  </div> ";
		$("body").append(content);
	}
...
$("#comment_form").submit(function() {
	...
	if (role==1) {
		...
		//未登陆
		var noEmail = $("#author_email").val();//未登陆的邮箱
		var noName = $("#author_name").val();//未登陆的昵称
		if(noEmail.search("hfans@foxmail.com")!=-1 | noName=="幻凡ss"){
			var theResponse = window.prompt("想冒充博主？不存在的，说个笑话让我听听....","做人还是踏实一点好");
			if (theResponse==21587980) {
				//输入正确
					$.ajax({
						async : false,
						type : "POST",
